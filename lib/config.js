export function llmSettings(env = process.env) {
  const provider = (env.STRUCTURED_LLM_PROVIDER || (env.GEMINI_API_KEY ? "gemini" : env.OPENAI_API_KEY ? "openai" : "gemini")).trim().toLowerCase();
  const llmApiKey = (env.STRUCTURED_LLM_API_KEY || (provider === "gemini" ? env.GEMINI_API_KEY : env.OPENAI_API_KEY) || "").trim();
  const fallbackProvider = (env.STRUCTURED_LLM_FALLBACK_PROVIDER || (llmApiKey ? provider : "")).trim().toLowerCase();
  return {
    llmProvider: provider,
    llmApiKey,
    llmModel: (env.STRUCTURED_LLM_MODEL || (provider === "gemini" ? "gemini-3.8-flash" : "gpt-5.4-mini")).trim().replace(/^models\//, ""),
    ...
    llmFallbackModel: (env.STRUCTURED_LLM_FALLBACK_MODEL || (fallbackProvider === "gemini" ? "gemini-3.5-flash-lite" : "gpt-4.1-mini")).trim().replace(/^models\//, "")
  };
}
