export function llmSettings(env = process.env) {
  const provider = (env.STRUCTURED_LLM_PROVIDER || (env.OPENAI_API_KEY ? "openai" : env.GEMINI_API_KEY ? "gemini" : "openai")).trim().toLowerCase();
  const llmApiKey = (env.STRUCTURED_LLM_API_KEY || (provider === "gemini" ? env.GEMINI_API_KEY : env.OPENAI_API_KEY) || "").trim();
  const fallbackProvider = (env.STRUCTURED_LLM_FALLBACK_PROVIDER || (provider === "openai" && llmApiKey ? "openai" : provider === "gemini" && env.OPENAI_API_KEY ? "openai" : "")).trim().toLowerCase();
  return {
    llmProvider: provider,
    llmApiKey,
    llmModel: (env.STRUCTURED_LLM_MODEL || (provider === "gemini" ? "gemini-2.5-flash" : "gpt-5.4-mini")).trim().replace(/^models\//, ""),
    llmFallbackProvider: fallbackProvider,
    llmFallbackApiKey: (env.STRUCTURED_LLM_FALLBACK_API_KEY || (fallbackProvider === provider ? llmApiKey : fallbackProvider === "gemini" ? env.GEMINI_API_KEY : fallbackProvider === "openai" ? env.OPENAI_API_KEY : "") || "").trim(),
    llmFallbackModel: (env.STRUCTURED_LLM_FALLBACK_MODEL || (fallbackProvider === "gemini" ? "gemini-2.5-flash" : "gpt-4.1-mini")).trim().replace(/^models\//, "")
  };
}
