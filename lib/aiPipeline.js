...
export function geminiSchema(schema) {
  // Gemini supports a JSON Schema subset. Length constraints remain enforced by AJV.
  if (Array.isArray(schema)) return schema.map(geminiSchema);
  if (!schema || typeof schema !== "object") return schema;
  return Object.fromEntries(Object.entries(schema).filter(([key]) => !["minLength", "maxLength"].includes(key)).map(([key, value]) => [key, geminiSchema(value)]));
}
...
export async function providerFailure(response) {
  ...
  const invalidKey = /api.?key.*(not valid|invalid|expired)|incorrect api key|invalid authentication/.test(message);
  ...
  if (response.status === 400) return new AppError("The AI provider rejected the request format. Your notes are unchanged; the workspace owner needs to check the AI configuration.", 502, "AI_REQUEST_REJECTED");
...
async function requestOutput(provider, key, model, input, timeoutMs, signal) {
  ...
  const body = gemini ? {
    ...
    generationConfig: {
      ...
      responseMimeType: "application/json", responseJsonSchema: geminiSchema(outputSchema),
  ...
  if (!response.ok) {
    ...
    console.warn("kaarya_ai_provider_error", { provider, http_status: response.status, code: error.code });
    throw error;
  }
