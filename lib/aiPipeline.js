...
export function aiAttemptPlan(settings = config) {
  const primary = { role: "primary", provider: settings.llmProvider, key: settings.llmApiKey, model: settings.llmModel, timeoutMs: 36000 };
  ...
  const fallbackModel = settings.llmFallbackModel || (fallbackProvider === "gemini" ? "gemini-2.5-flash" : settings.validatorModel || "gpt-4.1-mini");
  const attempts = [primary, { role: "fallback", provider: fallbackProvider, key: fallbackKey, model: fallbackModel, timeoutMs: 16000 }];
