...
export function geminiSchema(schema) {
  ...
  return Object.fromEntries(Object.entries(schema)
    .filter(([key]) => !["minLength", "maxLength", "additionalProperties"].includes(key))
...
export async function providerFailure(response) {
  ...
  const invalidKey = /api.?key.*(not valid|invalid|expired|leaked|blocked|suspended)|incorrect api key|invalid authentication/.test(message);
...
async function requestOutput(provider, key, model, input, timeoutMs, signal) {
  ...
  if (!response.ok) {
    ...
    console.warn("kaarya_ai_provider_error", { provider, model: /^[a-z0-9._-]{1,100}$/i.test(model) ? model : "invalid", http_status: response.status, code: error.code, fields: error.requestFields || [] });
    throw error;
  }
