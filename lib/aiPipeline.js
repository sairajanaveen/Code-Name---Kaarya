...
export async function extractAccountability({ meeting = {}, payload, instruction = "", previous = null, onStage = () => {}, signal }) {
  ...
  const transcript = await prepareTranscript(payload.raw_notes);
  ...
  const input = {
    ...
    transcript: transcript.text, ...(instruction ? { user_correction: instruction, previous_draft: previous } : {})
  };
  ...
  return { ...checked, processing: { provider, model, duration_ms: Date.now() - started, source_characters: payload.raw_notes.length, transcript_format: transcript.format, caption_count: transcript.cue_count, attempts } };
}
