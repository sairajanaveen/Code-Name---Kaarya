...
export async function supabaseFetch(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    if (["PGRST202", "PGRST204", "42703", "42P01"].includes(data?.code)) throw new AppError("The workspace needs the latest database update. Your draft is still available to copy.", 503, "SCHEMA_UPDATE_REQUIRED");
    const quotaErrors = {
      DAILY_LIMIT: ["Today's Free meeting has been used. Your allowance resets at midnight India time. Existing meetings remain available.", 429],
      MONTHLY_LIMIT: ["Your monthly meeting allowance has been used. Existing meetings remain available.", 429],
      STORAGE_FULL: ["Your meeting history is full. Export and delete a meeting to make room; nothing has been deleted automatically.", 409],
      REFINEMENT_LIMIT: ["The AI refinement allowance has been used. You can still edit every field manually.", 429],
      REQUEST_IN_PROGRESS: ["This draft is still processing. Retry shortly with the same notes to recover it.", 409],
      REQUEST_CONFLICT: ["This request has changed. Start a new draft and try again.", 409],
      REQUEST_DELETED: ["This meeting was deleted. Its generation allowance has already been used.", 410],
      REQUEST_EXPIRED: ["This request expired. Retry to check your saved result.", 409],
      INPUT_LIMIT: ["This transcript exceeds the 8 MiB safety limit. Nothing was truncated.", 400],
      JOB_ACTIVE: ["A transcript is already in progress. Resume or discard it before starting another.", 409],
      JOB_NOT_READY: ["Finish uploading the transcript before processing it.", 409],
      SECTION_RETRIES_EXHAUSTED: ["This section failed three times. Completed sections are saved; ask the workspace operator to check the AI configuration before retrying.", 503],
      NOT_OWNER: ["Meeting not found in your workspace.", 404],
      DELETE_BLOCKED: ["This meeting has an attached legacy file or scheduled delivery. Contact the workspace operator before deleting it.", 409]
    };
    const known = Object.keys(quotaErrors).find((code) => message.includes(code));
    if (known) throw new AppError(quotaErrors[known][0], quotaErrors[known][1], known);
...
export async function listMeetings(userId, offset = 0) {
  if (!userId) throw new AppError("Sign in to view meetings.", 401);
  return supabaseFetch(`/rest/v1/meetings?created_by=eq.${encodeURIComponent(userId)}&select=id,title,meeting_date,status,created_at,updated_at&order=created_at.desc,id.desc&limit=30&offset=${offset}`);
}
...
export async function listTasks(userId, meetingId) {
  ...
  const path = `/rest/v1/action_items?select=*,meetings!inner(created_by)&meetings.created_by=eq.${encodeURIComponent(userId)}${filter}&order=due_date.asc,id.asc`;
  if (!meetingId) return supabaseFetch(path + "&limit=200");
  const tasks = [];
  for (let offset = 0; offset < 2000; offset += 500) {
    const batch = await supabaseFetch(path + "&limit=500&offset=" + offset);
    tasks.push(...batch);
...
export function validTaskToken(token) {
  return typeof token === "string" && /^(?:[a-f0-9]{32}|[a-f0-9]{36})$/.test(token);
}

export async function getTaskByUpdateToken(token) {
  if (!validTaskToken(token)) return null;
...
export async function updateTaskByToken(token, updates = {}) {
  if (!validTaskToken(token)) throw new AppError("Task not found.", 404);
