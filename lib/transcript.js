...
async function parseTranscript(rawNotes) {
  if (typeof rawNotes !== "string" || rawNotes.length > MAX_TRANSCRIPT_LENGTH) {
    throw new AppError("The transcript exceeds the 8 MiB safety limit. Nothing has been cut off.", 400, "INPUT_LIMIT");
  }
...
export async function prepareTranscript(rawNotes, { tolerant = false } = {}) {
  try { return await parseTranscript(rawNotes); }
  catch (error) {
    if (!tolerant || error.code !== "INVALID_TRANSCRIPT") throw error;
    // Preserve every line if caption structure is broken; the model can still read the original.
    const content = rawNotes.replace(/^WEBVTT[^\n]*$/gm, "").replace(/^.*-->.*$/gm, "").replace(/^\s*\d+\s*$/gm, "");
    const issue = assessNotes(content);
    if (issue) throw new AppError(issue, 400, "WEAK_TRANSCRIPT");
    return { text: rawNotes, format: "unstructured_captions", cue_count: 0, warnings: ["The caption file had irregular formatting. All original lines were retained for processing."] };
  }
}
