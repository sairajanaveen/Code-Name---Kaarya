...
export function validateMeetingPayload(input = {}) {
  ...
  const payload = {
    ...
    review_before_send: Boolean(input.review_before_send),
    destination_channels: destinationChannels.length ? destinationChannels : ["email", "dashboard"]
  };

  const errors = [];
  const noteWordCount = payload.raw_notes.split(/\s+/).filter(Boolean).length;
  ...
  if (!payload.audio_url && !payload.attachment_url && noteWordCount > 0 && noteWordCount < 18) {
    errors.push("meeting notes are too short for reliable action items");
  }
