...
test("email contains compact tabular rows and latest edits", () => { const output = clone(exampleOutput); output.action_items[0].owner = "Edited owner"; const text = buildPostMeetingEmail({ meeting: { title: "Review" }, tasks: output.action_items }); assert.ok(text.includes("Action\tOwner\tTeam\tDue\tStatus")); assert.ok(text.includes("Edited owner")); assert.ok(!text.includes("AI-generated")); });
