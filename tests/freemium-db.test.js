...
before(async () => {
  ...
  for (const file of ["schema.sql", "upgrade-focus-flow.sql", "upgrade-freemium.sql"]) {
...
test("free size limit rejects work before inference", async () => {
  const id=randomUUID(); await assert.rejects(reserve(id,"generate",id,user,"a".repeat(64),20001),/INPUT_LIMIT/);
});
